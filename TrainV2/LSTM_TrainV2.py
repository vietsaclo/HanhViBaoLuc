...
if __name__ == '__main__':
    ...
    '''
    names, labels = cf.getVideoLabelNames_EachFolder(path=cf.DIR_INPUT_TRAIN)
    ...
    '''
    trainSet, labelSet = cf.getTrainSet_LabelSet(numItem=len(names), modelVGG16= modelVGG16, names= names, labels= labels)

    '''
        LAY MODEL LSTM DUOC DINH NGHIA,
        - Chi tiet dinh nghia mang LSTM tai config file
    '''
    modelLSTM = cf.getModelLSTM(num_classify= cf.NUM_CLASSIFY)
    modelLSTM.summary()

    '''
        BAT DAU HUAN LUYEN LSTM
    '''
    history = cf.fun_START_TRAINT_LSTM(modelLSTM= modelLSTM, trainSet= trainSet, labelSet= labelSet)
