...
def getTransferValue(pathVideoOrListFrame, modelVGG16):
...
def getTrainSet_LabelSet(numItem: int, modelVGG16, names, labels):
    count = 0
    trainSet = []
    labelSet = []
    while count < numItem:
        itemTrain = getTransferValue(pathVideoOrListFrame=DIR_INPUT_TRAIN + names[count], modelVGG16= modelVGG16)
        itemLable = onesHotLabel(label=labels[count])

        trainSet.append(itemTrain)
        labelSet.append(itemLable[0])

        lib.fun_print_process(count=count, max=numItem, mess='Video frame throw into VGG16 Model Processing: ')
...
def fun_START_TRAINT_LSTM(modelLSTM, trainSet, labelSet):
    history = modelLSTM.fit(np.array(trainSet[0:5]), np.array(labelSet[0:5]), epochs=EPOCH,
                        validation_data=(np.array(trainSet[5:]), np.array(labelSet[5:])),
